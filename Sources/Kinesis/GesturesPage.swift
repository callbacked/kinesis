...
struct GesturesPage: View {
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(spacing: 0) {
                if family == "swipe" {
                    ...
                    row(symbol: "dial.low", title: "pinch + turn") {
    ...
    private func assignment(_ label: String, symbol: String, last: Bool, selection: MacAction,
                            select: @escaping (MacAction) -> Void) -> some View {
        VStack(spacing: 0) {
            row(symbol: symbol, title: label.lowercased()) {
    ...
    private func row<Control: View>(symbol: String, title: String, @ViewBuilder control: () -> Control) -> some View {
        HStack(spacing: 12) {
